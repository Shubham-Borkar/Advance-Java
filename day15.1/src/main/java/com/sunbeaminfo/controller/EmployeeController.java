...
//		}
	...
	//signup req=all except id
	//resp=all except password
	@PostMapping("/signup")
	public ResponseEntity<?> addEmployee(@RequestBody @Valid SignupRequest request){
	
		return new ResponseEntity<>(empService.signUpEmp(request),HttpStatus.OK);
		//return ResponseEntity.status(HttpStatus.CREATED).body(empService.signUpEmp(request));
	}
	//get emp by sal ...("/salary/{minSal}")
	@PostMapping("/salary/{minSal}")
	public ResponseEntity<?> listEmpBySalary(@PathVariable double minSal){
		//return ResponseEntity.ok(empService.getEmpNameBySalary(minSal));
		return new ResponseEntity<>(empService.getEmpNameBySalary(minSal),HttpStatus.OK);
	}
